#!/usr/bin/env python3
"""Audit pira_nav symbol coverage and exact identity round trips."""
...
CURATED = {
    "markdown": {
        ("synthetic/document_project/guide.md", "heading1", "PIRA Guide"),
        (
            "synthetic/document_project/guide.md",
            "heading2",
            "PIRA Guide > Configuration",
        ),
    },
    "python": {
...
def main() -> int:
    ...
    for relative in files:
        ...
        if result.returncode:
            if "native parser found" in result.stderr and "--lsp" in result.stderr:
        ...
        backend_counts[backend.group(1) if backend else "native"] += 1
    ...
    for relative, kind, name, line, column, selector in records:
        ...
        for mode, target in targets.items():
            ...
            result = run(
                [str(binary), "show", target, *prefix, "--native"], data
            )
