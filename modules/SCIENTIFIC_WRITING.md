...
- Prefer direct claims and explicit logical connections.
- After drafting or polishing, perform a final audit at title, heading, sentence, paragraph, and document levels. Check motivation first: every title, heading, paragraph, and sentence must have a locally recoverable reason to appear. Establish that reason through the reader's active question or goal, a limitation that naturally motivates the next step, an explicit transition, or a clear logical dependency on nearby material. If the target reader cannot infer the reason, add the smallest meaningful connection, reorder the unit, or remove it; never invent a limitation merely to manufacture motivation.
- At every section boundary, read the preceding paragraph, the next heading, and the opening paragraph as one unit. The preceding text or the heading itself must establish why the next section is needed. The opening paragraph must answer that need without introducing a different purpose.
- A heading must not rely on its section body to retroactively explain why the section exists. If a heading contains an unfamiliar method, dataset, benchmark, acronym, or task-specific term, introduce its role before the heading or rewrite the heading around the reader's already-established goal. Define the term at its first retained use.
- Check meaningfulness after motivation: test whether each sentence and paragraph contributes to the document's main goal. If removing it would not make the target reader less able to understand that goal, its support, scope, or necessary limitations, remove it.
- Audit logic flow and target-reader clarity as a coupled loop. Logic: verify that each inference is supported, each sentence follows from and advances its context, each paragraph has a distinct role and connects coherently, and the overall argument contains no gaps or contradictions. Clarity: verify that the intended meaning is readily recoverable by the inferred target reader, required context appears before use, and technically correct wording is not needlessly opaque. Any edit made for either logic or clarity must trigger a local recheck against the other criterion; repeat until both criteria pass in one complete pass with no further edits. Remove filler, repetition, and metacommentary; retain decision-relevant evidence, concessions, and limitations.
...
- For multi-section documents and series, perform an outline-flow pass using only each title, heading, and the paragraphs immediately surrounding every section boundary. Verify that the resulting sequence forms a motivated progression, every named concept has already earned the reader's attention, and no heading merely labels content whose purpose becomes clear only afterward.
