...
2. Gather only the needed context; if the request is ambiguous or unclear, or context is insufficient, ask for clarification before answering or implementing.
...
## Research Loop
1. Gather online information whenever proper; default to broad-first and go deep or full only on explicit request.
2. Collect and verify evidence per `~/agent/modules/RESEARCH_POLICY.md`.
...
3. Run tests per `~/agent/modules/CODING_STYLE.md`.
