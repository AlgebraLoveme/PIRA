...
- After repairing an annotation, reinspect the entire figure. The repair fails if the label or its backing changes the perceived extent, connectivity, topology, or comparison of any semantic shape, even when the text is legible.
...
- Treat findings as conservative review warnings, not rejection criteria. Inspect every cited label in the final-use render and fix confirmed low contrast, clipping or masking, text overlap, or stroke intrusion. Resolve text conflicts without sacrificing label legibility or the underlying semantic geometry. Prefer moving the label into unused space; if relocation weakens association, use a short leader line or callout. Use a tightly padded halo or backing only as a last resort over non-informative background. It must not hide or visually fragment data-bearing shapes, boundaries, intersections, arrows, or meaningful area or overlap relationships. Never silence a checker warning merely by covering the interfering element.
